"""
infer_realtime_2d.py — Real-Time Dual-Camera 2D Multi-View Posture Inference Prototype
Performs live synchronized pose tracking, target participant selection, Reject Gate checks,
36-feature extraction, and XGBoost posture classification with HUD visualization.

Usage:
  # With physical dual webcams (CAM01=0, CAM02=1):
  ...
  python 04_scripts/inference/infer_realtime_2d.py --test-mode
"""
...
class ThreadedCamera:
    ...
    def __init__(self, cam_idx: int, width: int = 640, height: int = 480):
        ...
        # Try DirectShow on Windows for fastest start, fallback to default
        if sys.platform.startswith("win"):
...
def build_realtime_hud(
    ...
    # 1. Top HUD Bar (Dark transparent banner)
    hud_h = 70
    overlay = canvas.copy()
    ...
    # Text headers
    cv2.putText(canvas, "CAM01: FRONTAL", (15, 25), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (200, 200, 200), 1)
    cv2.putText(canvas, f"CAM02: LATERAL ({lateral_side.upper()})", (w1_r + 15, 25), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (200, 200, 200), 1)

    # Posture status and color coding
    ...
    if status == "VALID":
        if pred == "upright":
            badge_color = (50, 205, 50)  # Lime green
        elif "leaning" in pred:
            badge_color = (0, 165, 255)  # Orange
        else:
            badge_color = (0, 215, 255)  # Gold/Yellow for slouching
    ...
    cv2.putText(canvas, "MODE: 2D Multi-View XGBoost", (canvas_w - 280, 58), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (180, 180, 180), 1)
...
def run_realtime_2d(
    ...
    print("=" * 80)
    print("  LAUNCHING REAL-TIME 2D MULTI-VIEW POSTURE INFERENCE")
    print("=" * 80)
    ...
    if test_mode or test_cam01:
        # Load test images for simulated stream
        p1 = Path(test_cam01) if test_cam01 else PROJECT_ROOT / "02_data/private_raw/S024/SE01/CAM01/S024_SE01_CAP000852_CAM01.jpg"
        p2 = Path(test_cam02) if test_cam02 else PROJECT_ROOT / "02_data/private_raw/S024/SE01/CAM02/S024_SE01_CAP000852_CAM02.jpg"
        ...
        try:
            ...
            p1 = PROJECT_ROOT / "02_data/private_raw/S024/SE01/CAM01/S024_SE01_CAP000852_CAM01.jpg"
            p2 = PROJECT_ROOT / "02_data/private_raw/S024/SE01/CAM02/S024_SE01_CAP000852_CAM02.jpg"
    ...
    window_name = "Mitigasi Skoliosis — Real-Time 2D Multi-View (XGBoost)"
    ...
    try:
        while True:
            ...
            # Compose visual HUD
            hud_view = build_realtime_hud(f1, f2, result, avg_fps, lateral_side)
            ...
            if frame_count % 15 == 0:
                ...
                print(f"[Frame {frame_count:04d}] Posture: {pred_str:<16} | Conf: {conf_str:<12} | FPS: {avg_fps:4.1f}")
...
def main():
    parser = argparse.ArgumentParser(description="Real-Time 2D Multi-View Posture Inference")
    ...
    parser.add_argument("--lateral-side", type=str, default="right", choices=["left", "right"], help="Lateral side of CAM02")
