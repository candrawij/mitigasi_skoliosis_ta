"""
train_private_xgboost_2d.py — Train and Evaluate 2D Multi-View XGBoost (6 Classes)
...
Outputs in 07_results/experiments/private_final/2d/:
  - fold_metrics.csv
  - oof_predictions.csv
...
OUT_DIR = PROJECT_ROOT / "07_results" / "experiments" / "private_final" / "2d"
OUT_DIR.mkdir(parents=True, exist_ok=True)


def train_2d_xgboost():
    print("=" * 80)
    print("  STEP 10: TRAIN XGBOOST 2D MULTI-VIEW (SUBJECT-AWARE 5-FOLD CV)")
    print("=" * 80)
    ...
    # Parameter distributions for inner tuning
    param_dist = {
    ...
    for fold in range(5):
        ...
        search = RandomizedSearchCV(
            ...
            cv=inner_cv,
            random_state=42,
            n_jobs=1,  # GPU already uses parallel execution
            verbose=0
        )
    ...
    print("\n" + "=" * 80)
    print("  XGBOOST 2D MULTI-VIEW — FINAL 5-FOLD OOF RESULTS")
    print("=" * 80)
    ...
    plt.title(f"Confusion Matrix: XGBoost 2D Multi-View (OOF N={len(df_oof)}, Macro F1={oof_f1_mac:.4f})", fontsize=12)
